...
router.get('/website-mau',async function(req, res, next) {
  ...
  var temp =  caches.websiteinfo[hostname];
    ...
    if(hostname=='tnsvn.online'){
      website.gettemplatewebsites(8,async function(error,templatewebsite){
          testimonials.getcounttestimonialsbycustomer(3,websiteinfo.customer_id,async function(error,testimo){
            if(websiteinfo){
              var customer_username = websiteinfo.customer_username;
              var cusbanner = await getbanner(websiteinfo.customer_id);
      ...
      if(websiteinfo.website_url===hostname){
          ...
          Banners.getbannerbycustomer(websiteinfo.customer_id,function(err, cusbanner){
            res.render('content/'+customer_username+'/index', {
...
function getbanner(customer_id) {
  return new Promise((resolve, reject)=>{
    Banners.getbannerbycustomer(customer_id,function(err, banners){
        if (banners) {
          resolve(banners);
        }
        else{
...
router.post('/registertouse',async function(req, res, next) {
  ...
  customers.createcustomers(newcustomers,function(err, custo){
    if(err){
      ...
      let mailOptions = {
              from: '"Smartvas website" <admin@smartsign.com.vn>', // sender address
